...
function createCard(book) {
  ...
  name_div.style.cssText =
    "font-weight: bold; font-family: monospace; font-size: 24px; text-align: center;";
  ...
  bttn.addEventListener("click", () => {
    if (book.status) {
      bttn.textContent = "Not Read";
      localStorage.getItem(bookid)["status"] = false;
    } else {
      bttn.textContent = "Read";
      localStorage.getItem(bookid)["status"] = true;
    }
  });
...
window.addEventListener("load", display_all_cards);
